...
```python
"""스킬 목록 및 상세 정보 API -- .Codex/skills/ 디렉토리를 스캔"""
import re
from pathlib import Path
...
SKILLS_DIR = Path(__file__).resolve().parent.parent.parent / ".Codex" / "skills"
...
```python
"""플러그인(MCP 서버) 관리 API -- .Codex/settings.local.json의 mcpServers를 읽고 쓴다"""
import json
from pathlib import Path
from fastapi import APIRouter, HTTPException
from pydantic import BaseModel
...
SETTINGS_FILE = Path(__file__).resolve().parent.parent.parent / ".Codex" / "settings.local.json"
