...
| 2026-05-17 | 디자인샘플 admin 페이지 — voltagent/awesome-design-md (MIT, 71개 DESIGN.md) 통합. 사이드바 Claude Code 그룹 4번째 메뉴 + 일람 페이지 (카드 그리드/검색/카테고리 chip 필터/sync 배지) + 디테일 페이지 (좌 ReactMarkdown / 우 getdesign.md iframe 8s 폴백 / 5 액션: 복사·다운로드·토큰 프리뷰·외부 링크 2개). 토큰 프리뷰는 CSS variables 를 `.mdPane` 스코프에만 inject (사이드바·탑바 무영향) | feat | frontend/src/components/layout/Sidebar.jsx, frontend/src/App.jsx, frontend/src/pages/admin/AdminDesignMd.{jsx,module.css}, frontend/src/pages/admin/AdminDesignMdDetail.{jsx,module.css} |
| 2026-05-17 | 백엔드 디자인샘플 인프라 — `models/design_md.py` (DesignMd / DesignMdSyncMeta 두 테이블), Alembic 리비전 `2026_05_17_design_md`, `services/design_md_sync.py` (GitHub README + git tree 페치 → DESIGN.md 71개 동시 10 raw 페치 → 토큰 추출 → upsert + tree-기준 delete + sync meta), `services/design_md_tokens.py` (hex 색상/font-family·fontFamily 빈도순 추출), `routers/design_md.py` (4 admin endpoint, BackgroundTasks sync), `main.py` lifespan 24h 자동 sync 훅 | feat | backend/models/design_md.py, backend/alembic/versions/2026_05_17_design_md.py, backend/services/design_md_{sync,tokens}.py, backend/routers/design_md.py, backend/main.py, backend/alembic/env.py |
| 2026-05-17 | pytest 인프라 신규 도입 (codebase 최초) — `backend/tests/` + conftest.py (sys.path 셋업) + `test_design_md_tokens.py` 12 테스트 (hex frequency/case norm/12 limit/empty/3-digit/8-digit alpha 제외 + font-family/unquoted/dedupe/empty/limit/JSX camelCase). requirements.txt 에 `pytest>=8.0`, `pytest-asyncio>=0.23` 추가 | feat | backend/requirements.txt, backend/tests/{__init__.py,conftest.py,test_design_md_tokens.py} |
| 2026-05-17 | docs/superpowers/{specs,plans}/ 디렉토리 신규 — `2026-05-17-design-md-samples-design.md` (399 라인 spec) + `2026-05-17-design-md-samples.md` (1922 라인 plan, 11 task). brainstorming → writing-plans → subagent-driven-development 풀 워크플로 적용 (실행 회고는 work-log 참조) | docs | docs/superpowers/specs/2026-05-17-design-md-samples-design.md, docs/superpowers/plans/2026-05-17-design-md-samples.md |
