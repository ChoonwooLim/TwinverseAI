...
4. 계획에 따라 구현: Vault 스켈레톤 → MVP 시드 → 배포 스크립트 → /end 확장 → CLAUDE.md 규칙 추가 → Claude Desktop 연동 가이드

---

## 12. Claude Desktop 연동 (선택 · 병행 사용 지원)

Steven 은 같은 프로젝트를 Claude Code (CLI) 와 Claude Desktop (앱) 에서 병행 사용. Wiki 를 두 클라이언트에서 모두 참조 가능하게 설계.
...
**결론**: 읽기/편집은 MCP 설정으로 동등. `/end` 자동 제안은 Code 전용 — Desktop 에서는 수동 업데이트.

### 12.2 권장 셋업 (Desktop 측)

**① Obsidian MCP 서버 설치** — 읽기·쓰기 모두 지원:

`claude_desktop_config.json` (경로: `%AppData%\Claude\claude_desktop_config.json`) 에 추가:
...
- Obsidian 의 "Local REST API" 커뮤니티 플러그인 설치 필요 (Obsidian 앱 실행 중일 때만 작동)
- 대안: `@modelcontextprotocol/server-filesystem` 로 `C:\WORK\llm-wiki\` 만 노출 (Obsidian 앱 없이도 됨, API 한정적)

**② Claude Desktop Project + 커스텀 지시**
...
- 설명·이유·패턴은 Wiki 가 SSOT
```

Claude Code 의 `AI_WIKI.md` 포인터를 Desktop 에서 대체하는 장치.

**③ GitHub Project Knowledge 연동** (선택)

Claude Pro/Max 사용 시 llm-wiki GitHub repo 를 Claude Desktop Project Knowledge 로 연결 → MCP 없이도 Wiki 내용이 대화 기본 컨텍스트에 포함. 업데이트는 분 단위 지연, 편집은 여전히 로컬 Obsidian 에서.

### 12.3 Wiki 내부 페이지로 문서화

MVP 시드 스텁 1 개 추가: **`40-Tools/Claude-Desktop.md`**
- 위 3 가지 셋업 가이드 전문
- 본인 `claude_desktop_config.json` 의 현재 MCP 서버 목록 · 트러블슈팅 노트
- Obsidian Local REST API 플러그인 설치 방법

### 12.4 `/end` 키워드 매핑 추가

`.wiki-sync.json` 의 `keywords` 에 추가:

```json
"claude[.\\s-]?desktop|mcp\\s*server": ["40-Tools/Claude-Desktop.md"]
```

Desktop 연동 관련 커밋이 감지되면 해당 페이지가 업데이트 후보로 제안됨.

### 12.5 구현 계획에 반영

- MVP 시드 13 페이지 → **14 페이지** (Claude-Desktop.md stub 추가, 초기에는 가이드 ~80 줄)
- `claude_desktop_config.json` 자체는 repo 밖이므로 **수정 대상 아님** — 수동 설정 가이드만 Wiki 에 박제
- 검증 체크리스트: Claude Desktop 에서 obsidian-mcp 툴 호출로 `00-Home.md` Read 성공 확인
