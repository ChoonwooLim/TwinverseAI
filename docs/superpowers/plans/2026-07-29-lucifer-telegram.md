...
- [x] **Step 4: 종단 검증 — 아웃바운드 동작, 인바운드는 `--channels` 세션 opt-in 필요**

**아웃바운드(세션 → 텔레그램): 동작.** `reply` 툴로 메시지 전송 성공(id 11, 14).

**인바운드(텔레그램 → 세션): `--channels` 플래그 없이는 동작하지 않는다.**
처음에 "Claude Code 2.1.220 이 채널 알림을 지원하지 않는다" 고 결론냈으나 **틀렸다.**
claude.exe 바이너리를 직접 뜯어 게이트 로직을 복원한 결과, 지원은 존재하고
**세션별 opt-in 게이트**가 막고 있었다:

```js
// 바이너리에서 복원한 게이트 (5단계)
if (!server.experimental["claude/channel"]) skip("capability")
if (provider !== "firstParty")              skip("provider")   // Bedrock/Vertex/Foundry 불가
if (!featureFlag)                           skip("disabled")
if (orgPolicy.channelsEnabled !== true …)   skip("policy")     // managed settings 있는 조직만 해당
...
**디버깅 기록 (다음에 헤매지 않기 위해):**

| 확인 | 결과 |
|---|---|
| "입력 중" 표시 | 게이트 통과의 증거 — `sendChatAction` 은 `gate()` drop 지점(:902) 뒤(:940) |
| 폴링 슬롯 | 세션마다 server.ts 를 띄우고 최신 세션이 PID 파일로 이전 poller 를 SIGTERM — **인바운드는 슬롯을 쥔 세션으로만 간다** |
...
**지니·로이는 영향 없다** — OpenClaw 게이트웨이가 직접 처리하므로 이 게이트와 무관하다.
