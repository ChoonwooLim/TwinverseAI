...
- [x] **Step 4: 종단 검증 — 아웃바운드만 동작, 인바운드는 하네스 미지원**

**아웃바운드(세션 → 텔레그램): 동작.** `reply` 툴로 메시지 전송 성공(id 11, 14).

**인바운드(텔레그램 → 세션): 동작하지 않음.** 설정 문제가 아니라 Claude Code 2.1.220 이
플러그인의 알림을 대화에 올리지 않는 것이다. 아래가 실측 근거다.

| 확인 | 결과 |
|---|---|
| `gate()` 통과 여부 | **통과** — `sendChatAction("typing")` 은 `server.ts:940`, gate drop 은 `:902`. "입력 중" 이 떴다는 건 게이트를 지났다는 뜻 |
| 알림 발송 | `:955` 의 `notifications/claude/channel` 까지 도달 |
| 봇 폴링 | 정상 (`getWebhookInfo.pending_update_count = 0`) |
| VS Code 확장 | 표시 안 됨 |
| 터미널 CLI | **표시 안 됨** (하네스 차이가 아님) |
| `claude --help` | channel 관련 옵션 없음 — 켤 스위치가 없다 |
| 플러그인 버전 | 0.0.6 이 마켓플레이스 최신, 업데이트 없음 |

디버깅 중 한 번 **"VS Code 확장의 한계이니 CLI 로 옮기면 된다" 고 잘못 짚었다.** CLI 에서도
동일하게 실패했으므로 하네스 종류가 아니라 기능 지원 문제다. "입력 중" 표시는 게이트
통과/미통과를 가르는 좋은 신호였다 — 다음에도 이 지점부터 확인할 것.

**현재 운용 방침:** 클로드 봇은 아웃바운드 전용(세션에서 텔레그램으로 알림·결과 전송).
인바운드가 필요해지면 플러그인 업데이트를 다시 확인한다.

**지니·로이는 영향 없다** — OpenClaw 게이트웨이가 직접 처리하므로 하네스와 무관하다.
